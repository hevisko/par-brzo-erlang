...
%-vsn('$Revision: 1.2 $').
-vsn('$Id: ddi.erl,v 1.2 2010/07/02 18:38:03 hvisage Exp $').
...
hv_brz(RE, Sigma, available, N) ->
    TimeOut = 1000,
    Res = self(),
    Dist = spawn(fun () -> hv_dist_avail_start(TimeOut, Sigma, Res, N)
		 end),
    WiP = ets:new(table,[set]),%Only the first (and last) one would be "empty"
    Finish = [],
	Dlist=ets:new(dlist,[set]),ets:insert(Dlist, {RE}),%Start state.
	Delta=ets:new(dlist,[set]),
	clear_mbox(),
	put(start,RE),
    hv_brzp_null(RE, Sigma, Dist, WiP,cont, Finish, Dlist, Delta);
hv_brz(RE, Sigma, roundrobin, N) ->
	...
	clear_mbox(),
	put(start,RE),	
     Dist = spawn(fun () -> hv_dist_rr_start(TimeOut, Sigma, Res, N) end),
   hv_brzp_null(RE, Sigma, Dist, WiP,cont, Finish, Dlist, Delta).
...
hv_brz(RE,Sigma,rr_spec) ->
	...
	clear_mbox(),
	put(start,RE),
	Dist=spawn(fun() -> hv_dist_rr_spec_start(TimeOut,Sigma,Res) end),
	hv_brzp_null(RE,Sigma,Dist,WiP,cont,Finish,Dlist,Delta).
...
list_start_servers(0,_Sigma,_Res) -> [];
list_start_servers(N,Sigma,Res) -> 
	[spawn(fun()->hv_rr_rec("Receiver "++[N+$0],Sigma,Res) end)|list_start_servers(N-1,Sigma,Res)].

%Number of servers variable, should make that a number to pass too, but 
% for the moment this is adequate to test etc.
hv_dist_rr_start(TimeOut,Sigma,Res,N) -> 
	hv_dist_rr(list_start_servers(N,Sigma,Res),TimeOut).

%Two specified servers
hv_dist_rr_spec_start(TimeOut,Sigma,Res)->
	Rec1=spawn(fun() ->hv_rr_rec("Rec1",Sigma,Res) end),
	Rec2=spawn(fun() ->hv_rr_rec("Rec2",Sigma,Res) end),
...
hv_brzp_null(receive_only,Sigma,Dist,WiP,'$end_of_table',Finish,Dlist,Delta) ->
	...
	ets:delete(WiP),
	Dist!{stop},
	Dl=ets:tab2list(Dlist),ets:delete(Dlist),
	De=ets:tab2list(Delta),ets:delete(Delta),
	#dfa{states=lists:sort(Dl),symbols=Sigma,start=get(start),transition=dict:from_list(De),finals=Finish};
...
hv_brzp_null(receive_only,Sigma,Dist,WiP,_,Finish,Dlist,Delta) ->
	...
	receive
		{rd,E,I,DDI} -> %io:format("brzp_null_2:"),io:write({rd,E,I,DDI}),io:format("~n"),
			...
			ets:insert(Delta,{{E,I},DDI}),
			case ets:member(Dlist, DDI) of
				true -> 
						hv_brzp_null(receive_only,Sigma,Dist,WiP,ets:first(WiP),Finish,Dlist,Delta);
				false -> Dist!{process,[ddi,DDI]},
							ets:insert(Dlist,{DDI}),
							lists:foreach(fun(X)->ets:insert(WiP, {{rd,DDI,X}}) end, Sigma),
							ets:insert(WiP,{{null,DDI}}),
						 %erlang:yield(),
						 hv_brzp_null(receive_only,Sigma,Dist,WiP,cont,Finish,Dlist,Delta)
			end;
		{null,E,true} -> %io:format("brzp_null_2: ~p true~n",[E]),
			case	ets:member(WiP, {null,E}) of
				true ->	ets:delete(WiP,{null,E}),
									hv_brzp_null(receive_only,Sigma,Dist,WiP,ets:first(WiP),[E|Finish],Dlist,Delta);	% Add nullable states to F
				false -> %io:write({E,true}),io:nl(),
						 			hv_brzp_null(receive_only,Sigma,Dist,WiP,ets:first(WiP),Finish,Dlist,Delta) % Add nullable states to F
			end;
...
hv_brzp_null(E,Sigma,Dist,WiP,_,Finish,Dlist,Delta) ->
	...
	lists:foreach(fun(X)->ets:insert(WiP, {{rd,E,X}}) end, Sigma),
	ets:insert(WiP,{{null,E}}),
	%erlang:yield(),
	hv_brzp_null(receive_only,Sigma,Dist,WiP,cont,Finish,Dlist,Delta).
...
%===============================================
hv_rec_available(Timeout,Sigma,Name,Res,Dist) ->
	hv_rec_available(Timeout,Sigma,Name,Res,Dist,queue) .
...
hv_rec_available(Timeout,Sigma,Name,Res,Dist,queue) ->
	...
	receive
		...
		{process,Params} -> %io:format("Pr:~p~n",[Params]),
							process(Res,Params,Sigma),
							%erlang:yield(),
							hv_rec_available(Timeout,Name,Res,Dist,queue);
		Other1 -> throw(Other1)
		after 0 -> Dist!{available,self()},hv_rec_available(Timeout,Sigma,Name,Res,Dist,wait) %Nothing in queue, so we let the Distributor know
	end;
hv_rec_available(Timeout,Sigma,Name,Res,Dist,wait) ->
...
process(Res,[ddi,E],Sigma) ->
	lists:foreach(fun(X)-> Res!{rd,E,X,mds:reduce(mds:deriv(E,X))} end,Sigma),
	Res!{null,E,mds:null(E)},true;
...
hv_dist_available(Timeout,[]) ->
	...
	receive
		...
		{stop} -> stopping_receivers([]),
				  io:format("Distributor stopping from empty state~n") %no available receivers to stop :(
		after Timeout ->
...
hv_dist_available(Timeout,[H|Tail])-> %At least have a PID to check
	...
	receive
		...
		{stop}-> stopping_receivers([H|Tail]); %Stop all the available receivers
		Other -> throw(Other)
		after Timeout ->
...
hv_dist_avail_start(Timeout,_Sigma,_Res,0)	->
		%io:format("Empty~n"),
		hv_dist_available(Timeout,[]);
hv_dist_avail_start(Timeout,Sigma,Res,N) when N>0 ->
	...
	Dist=self(),
	spawn(fun() -> hv_rec_available(Timeout,Sigma,"Receiver "++erlang:integer_to_list(N),Res,Dist) end),
	hv_dist_avail_start(Timeout,Sigma,Res,N-1).




%% $Log: ddi.erl,v $
%% Revision 1.2  2010/07/02 18:38:03  hvisage
%% After some tuning
%%
%% Revision 1.1  2010/07/02 13:46:16  hvisage
%% another method
%%
